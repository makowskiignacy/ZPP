...
def generate_starting_points(self, data, model, distance, directions=1000, steps=1000):
    ...
    if starting_points is None:
        test_logger.debug(f'Wrong starting points ({starting_points}) for params: distance={distance}, directions={directions}, steps={steps}')
    else:
        test_logger.debug(f'Successful starting points ({starting_points}) for params: distance={distance}, directions={directions}, steps={steps}')
    ...
    for dist in distances2check:
        for dir in directions2check:
            for steps in steps2check:
                ...
                if starting_points is None:
                    test_logger.debug(f'Wrong starting points ({starting_points})')
                else:
                    test_logger.debug(f'Successful starting points ({starting_points})')
                    # sys.exit(f'Wrong starting points ({starting_points}) for params: distance={dist}, directions={dirs}, steps={stps}')
                    ptsSet.append(starting_points)
                
                test_logger.debug(f"Checked for params: distance={dist}, directions={dir}, steps={steps}")
                test_logger.debug(f"Took {time_end - time_start}\n")
