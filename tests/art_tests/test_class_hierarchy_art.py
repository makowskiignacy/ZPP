...
class TestClassHierarchyArt:
    def test_class_hierarchy(self):
        ...
        assertion_vars = []
        assertion_var = 0
        logger.info("Próba utworzenia obiektu klasy Attack:\n****")
        try:
            AbsA = Attack()
            assertion_var += 1
            logger.info("Brak błędów.")
        ...
        logger.info("Próba utworzenia obiektu klasy ARTAttack:\n****")
        try:
            AbsA = ARTAttack({})
            assertion_var += 1
            logger.info("Brak błędów.")
        ...
        logger.info("Klasę konkretnego ataku można już zinstancjonować:\n****")
        try:
        ...
        for key, val in default_params.items():
            if changed_params[key] != val:
                logger.info("diff:", key, "default:", val, "changed:", changed_params[key])
            else:
                logger.info("same:", key, "=", val)
